...
function Login($input_parameter){
	...
	if( $num_rows == 0 ){
		...
		if( $result ){
			...
			$_SESSION['OSH']['NAME'] = $row['name'];
			$_SESSION['OSH']['EMAIL'] = $row['email'];
			$_SESSION['OSH']['PASSWORD'] = $row['password'];
			$_SESSION['OSH']['ROLES'] = $row['roles'];
...
function GetMasterLink(){
	...
	$master_link = "http://localhost/development_site/osh-v200";
	return $master_link;
	
}
function GetSiteTitle(){
	$site_title = "Open Swelab V200";
	return $site_title;
}
